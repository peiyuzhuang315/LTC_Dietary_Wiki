...
const EXPERTS = [
  ...
  { id:"assistive_tech", emoji:"🏠", color:"#558b2f",
    realRole:"輔具評估員", digitalTitle:"空間改造成型師",
...
function buildSystemPrompt(experts) {
  ...
  const secondary = experts[1];
  let inject = `\n\n【本題啟動的專家角色】\n${primary.persona}\n核心數位技能：${primary.skills.join('、')}。`;
  if (secondary) inject += `\n\n同時融入「${secondary.digitalTitle}」（${secondary.realRole}）的協作觀點，技能：${secondary.skills.join('、')}。`;
  inject += `\n回答時，語氣專業直接，展現該角色的專業深度。`;
  return base + inject;
}
...
回答規則：
1. 使用繁體中文，語氣專業直接
2. 絕對不說寒暄語、開場白、「您好！」「很高興為您說明」等，直接進入重點
3. 回答長度適中（200-400 字），條列清晰
...
async function submitQuestion() {
  ...
  try {
    ...
    document.getElementById("chat-answer-content").innerHTML = renderMd(cleanAnswer);
